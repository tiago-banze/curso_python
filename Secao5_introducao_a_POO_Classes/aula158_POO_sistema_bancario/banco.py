...
class Banco():
    ...
    def _checa_se_a_conta_e_do_cliente(self, cliente, conta):
        if conta is cliente.conta:
            print('_checa_se_a_conta_e_do_cliente', True)
            return True
        print('_checa_se_a_conta_e_do_cliente', False)
        return False
        ...
        return self._checa_agencia(conta) and \
            self._checa_cliente(cliente) and \
                self._checa_conta(conta) and \
                    self._checa_se_a_conta_e_do_cliente(cliente, conta)
    def __repr__(self):
...
if __name__ == '__main__':
    c1 = pessoas.Cliente('Tiago', 30)
    cc1 = contas.ContaCorrente(111, 222, 0, 0)
    c1.conta = cc1
    c2 = pessoas.Cliente('Banze', 18)
